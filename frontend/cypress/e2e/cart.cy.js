describe("Cart tests", () => {
  ...
  it("should not add an unavailable product in the cart", () => {
  ...
  context("with an available product", () => {
    it("should add an available product in the cart and update the stock", () => {
      ...
      // cy.intercept("GET", "**/products/7").as("getProduct");
      let nstock;
      cy.visit("/products/7");
      // cy.wait("@getProduct")
      ...
      cy.wait("@getCart")
        ...
        .then((orderLines) => {
          const addedProduct = orderLines.find(
            (OrderLine) =>
              OrderLine.product.id === 7 && OrderLine.quantity === 1,
          );
    ...
    it("should not allow to add a product in the cart with a negative quantity", () => {
      // checkProductStock().then((productId) => {
      cy.visit("/products/7");
      // Taper un chiffre négatif dans le champ de quantité du produit
      cy.getBySel("detail-product-quantity").clear().type("-1");
      // Vérifier qu'on ne puisse pas valider la modification de la quantité en vérifiant que la classe du formulaire du produit soit invalide
    ...
    it("should not allow to add a product in the cart with a quantity superior to 20", () => {
      // checkProductStock().then((productId) => {
      cy.visit("/products/7");
